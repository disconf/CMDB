...
### P2（治理与体验）
- [ ] 发现审批流：新发现 → 待审批 → 入账（可选自动）
- [ ] 批量远程执行：对目标主机下发命令（只读白名单/审批）
- [ ] 审计：谁/何时/对哪些主机做了什么（安装/卸载/扫描/入账）
- [ ] 任务结果页：每次扫描的 发现/合并/冲突/失败 明细与日志

### P3（更广覆盖）
- [ ] K8s 资源采集器：集群→Node→Namespace→Workload→Pod 自动入 CMDB
